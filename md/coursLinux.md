...
Cet OS est célèbre pour son interprète de commande appelé *Terminal* ou *Shell*.


En 1991, un étudiant finlandais **Linus TORVALD** envoie un mail avec une pièce-jointe contenant le noyau d'un système d'exploitation libre inspiré d'UNIX : **Linux**
...
Il existe une commande qui affiche le manuel / la documentation d'une autre commande passée en argument : `man`.
...
-------------
La commande `cd`(*change directory*) permet de se déplacer dans l'arborescencedes dossiers.
Sur Linux, le dossier **racine**(*root*) se note `/`.

On peut donner le chemin vers un dossier (ou un fichier) de manière : 
